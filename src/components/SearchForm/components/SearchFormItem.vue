<template>
  <component
    :is="column.search?.render ?? InputItem"
    :input-type="column.search?.el"
    v-bind="{ ...props.column.search?.props, ...placeholder, searchParam: _searchParam, clearable }"
    v-model.trim="_searchParam[column.search?.key ?? handleProp(column.prop!)]"
    :options="columnEnum"
    :props="props.column.fieldNames"
  >
    <template #default>
      <slot></slot>
    </template>
...
// 接收 enumMap (el 为 select-v2 需单独处理 enumData) 原因是 select-v2  不支持 props 属性指定 label,value,children字段
